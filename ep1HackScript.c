...
int main(int argc, char const *argv[]) {
    ...
    fclose (fp);
    fclose (fp2);
    if (cab->prox->escalonador == 1) {
      FILE *mudCont;
      mudCont = fopen ("tabelaEsc1.txt","a");
      if (mudCont!=NULL)
      {
        fprintf(mudCont,"%d %d %d\n", atoi(argv[4]), numMudancasContexto, nCumprimentoDeadline);
        fclose (mudCont);
      }
    } else if (cab->prox->escalonador == 2) {
      FILE *mudCont;
      mudCont = fopen ("tabelaEsc2.txt","a");
      if (mudCont!=NULL)
      {
        fprintf(mudCont,"%d %d %d\n", atoi(argv[4]), numMudancasContexto, nCumprimentoDeadline);
        fclose (mudCont);
      ...
      FILE *mudCont;
      mudCont = fopen ("tabelaEsc3.txt","a");
      if (mudCont!=NULL)
      {
        fprintf(mudCont,"%d %d %d\n", atoi(argv[4]), numMudancasContexto, nCumprimentoDeadline);
        fclose (mudCont);
...
void contador (processo *p) {
    ...
    struct timespec start, mid, midMestre, end, new_start;
    long int conta = 0;
    int n = 0;
    processo * q;
